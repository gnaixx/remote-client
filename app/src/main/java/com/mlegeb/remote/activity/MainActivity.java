...
public class MainActivity extends BaseActivity implements OnClickListener{
	...
	private void initViews(){
		...
		//读取保存的IP地址
		serverIpEdit.setText(SettingsUtil.getPref(this, Constants.IP_ADDR, Constants.DEF_IP_ADDR).toString());
		
		//读取保存的鼠标灵敏度
		Constants.mouseSensibility = Integer.parseInt(SettingsUtil.getPref(this, Constants.MOUSE_SENSI, 1));
		...
		public void onReceive(Context context, Intent intent) {
			if(intent.getAction().equals(Constants.RETURN_ACTION)){
				if(intent.getBooleanExtra("result", false)){
					...
					Constants.conn_address = serverIpStr;
					
					//保存IP地址
					SettingsUtil.savePref(MainActivity.this, Constants.IP_ADDR, Constants.conn_address);
					//跳转到菜单页面
	...
	public void onWindowFocusChanged(boolean hasFocus) {
		...
		if(true){
			...
			SettingsUtil.savePref(MainActivity.this, 
					Constants.FIRST_START_PAGE_1, false);
			isFirst = false;
		}
	}
